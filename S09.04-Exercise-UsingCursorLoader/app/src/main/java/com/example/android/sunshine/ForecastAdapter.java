...
//      TODO (8) Display the summary that you created above
        ...
        String description = SunshineWeatherUtils.getStringForWeatherCondition(
                mContext,
                mCursor.getInt(MainActivity.INDEX_WEATHER_CONDITION_ID)
        );

        String highLowString = SunshineWeatherUtils.formatHighLows(
                mContext,
                mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP),
                mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP)
        );

        String summaryText = dateString + " - " + description + " - " + highLowString;

        forecastAdapterViewHolder.weatherSummary.setText(summaryText);
    }
