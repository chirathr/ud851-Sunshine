...
//  TODO (20) Create a constant int to identify our loader used in DetailActivity
    ...
    private static final int ID_DETAIL_LOADER = 153;
...
//      TODO (13) Find each of the TextViews by ID
        dateTextView = (TextView) findViewById(R.id.tv_display_date);
        descriptionTextView = (TextView) findViewById(R.id.tv_display_description);
        highTextView = (TextView) findViewById(R.id.tv_display_high);
        lowTextView = (TextView) findViewById(R.id.tv_display_low);
        humidityTextView = (TextView) findViewById(R.id.tv_display_humidity);
        windTextView = (TextView) findViewById(R.id.tv_display_wind);
        pressureTextView = (TextView) findViewById(R.id.tv_display_pressure);
...
//      TODO (33) Store a forecast summary in mForecastSummary
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        String dateText = SunshineDateUtils.getFriendlyDateString(
                this,
                cursor.getLong(INDEX_WEATHER_DATE),
                false
        );
        ...
        highTextView.setText(
                SunshineWeatherUtils.formatTemperature(
                  this,
                  cursor.getDouble(INDEX_WEATHER_MAX_TEMP))
        );

        lowTextView.setText(
                SunshineWeatherUtils.formatTemperature(
                  this,
                  cursor.getDouble(INDEX_WEATHER_MIN_TEMP))
        );

        humidityTextView.setText(
                getString(R.string.format_humidity, cursor.getFloat(INDEX_WEATHER_HUMIDITY))
        );
        ...
        pressureTextView.setText(
                getString(R.string.format_pressure, cursor.getDouble(INDEX_WEATHER_PRESSURE))
        );

        mForecastSummary = String.format("%s - %s - %s/%s",
                dateText, description, highTextView.getText(), lowTextView.getText());
    }
